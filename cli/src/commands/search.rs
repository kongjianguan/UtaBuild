...
fn is_exact_match(title: &str, artist: Option<&str>, result: &SearchResult) -> bool {
    let title_match = {
        let clean_query = title.split('(').next().unwrap_or(title).trim().to_lowercase();
        let clean_result = result.title.split('(').next().unwrap_or(&result.title).trim().to_lowercase();
...
pub async fn execute(
    ...
    debug!("执行搜索: title={:?}, artist={:?}, page={}, select={:?}", title, artist, page, select);
    ...
    if let Some(index) = select {
        ...
        let process_result = searcher.process_song(&title, artist_ref).await;
        ...
        if selected_result.status == "success" {
            ...
            if let Some(output_path) = output {
                ...
                let filename = generate_default_filename(&selected_result.found_artist, &selected_result.found_title);
                write_output_to_file(&filename, &json_content)?;
                info!("已输出到文件: {}", filename);
            } else {
                println!("{}", json_content);
        ...
        if exact_matches.len() == 1 {
            ...
            info!("找到精确匹配: {} - {}", exact_result.title, exact_result.artist);
            info!("检查歌词缓存: {}", lyrics_url);
            ...
            let output = SearchOutput::new(
                ...
                artist,
                page,
                process_result.search_results.len() as u32,
