...
export function saveSearchHistory(historyItems) {
    try {
...
export function addSearchHistory(title, artist) {
    ...
    const existing = loadSearchHistory().filter((item) => historyKey(item.title, item.artist) !== newKey);
...
function fillSearchFromHistory(entry) {
    el('search-title').value = entry.title;
    el('search-artist').value = entry.artist || '';
    el('search-title').focus();
}
// ==================== Render ====================
export function renderSearchHistory() {
    const list = el('search-history-list');
    const empty = el('search-history-empty');
